...
export const Header: React.FC<HeaderProps> = ({ status, userEmail, onLogout }) => {
  ...
  return (
    <header className="flex justify-between items-center py-5 px-8 border-b border-slate-200/60 fixed top-0 right-0 left-64 z-10 glass-panel" style={{ backgroundColor: 'rgba(255, 255, 255, 0.75)' }}>
      {/* Greetings */}
      <div>
      ...
      <div className="flex items-center gap-4">
        ...
        {/* Theme Toggle */}
        <button className="p-2 rounded-lg bg-white hover:bg-slate-50 border border-slate-200 text-slate-500 hover:text-slate-800 transition-all cursor-pointer">
          <Sun size={16} />
        </button>
